...
def fetch_and_update():
    ...
    if orders_data:
        ...
        agg_datewise['Session_Difference'] = agg_datewise['Session'].diff()
        agg_datewise['Session_Difference_Percentage'] = agg_datewise['Session_Difference']/agg_datewise['Session']*-1

        #### Percentage change in session count
        agg_datewise['Revenue_Difference'] = agg_datewise['Revenue'].diff()
        agg_datewise['Revenue_Difference_Percentage'] = agg_datewise['Revenue_Difference']/agg_datewise['Revenue']*-1

        #### Percentage change in conversation rate
        agg_datewise['Conversion_Rate_Difference'] = agg_datewise['Conversion_Rate'].diff()
        agg_datewise['Conversion_Rate_Percentage'] = agg_datewise['Conversion_Rate_Difference']/agg_datewise['Conversion_Rate']*-1

        #### Percentage change in AOV
        agg_datewise['AOV_Difference'] = agg_datewise['AOV'].diff()
        agg_datewise['AOV_Difference_Percentage'] = agg_datewise['AOV_Difference']/agg_datewise['AOV']*-1

        ################################### Removing unnecessary columns #####################################
        agg_datewise.drop(['Session_Difference', 
                            'Revenue_Difference',
...
schedule.every().day.at("17:00").do(fetch_and_update)
